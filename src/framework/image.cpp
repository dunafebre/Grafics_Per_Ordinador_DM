...
{
    int dx = x1 - x0; //creem el vector horitzontal del punt A al punt B
    int dy = y1 - y0; //creem el vector vertical del punt A al punt B
    
    int d = std::max(abs(dx), abs(dy)); //calcula d com el catet més gran del triangle (tingueu en compte que d indica quants píxels hem de dibuixar)
    
    float vx = dx / (float)d; //vector director
    float vy = dy / (float)d; //vector director
    
    float x = x0; //inicialitzem les coordenades
    float y = y0; //inicialitzem les coordenades
    
    for(int i = 0; i < d; i++){
        SetPixel((int)floor(x), (int)floor(y), c);
        x += vx;
        y += vy;
    }
}
...
void Image::ScanLineDDA(int x0, int y0, int x1, int y1, std::vector<Cell>& table, int minY)
{
    ...
    for(int i = 0; i < d; i++){
        int index = (int)round(y) - minY;
        if(table[index].minx > x){
            table[index].minx = x;
        }
        if(table[index].maxx < x){
            table[index].maxx = x;
        }
        x += vx;
        y += vy;
